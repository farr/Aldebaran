...
/* partindo de uma tabella di numax, dnu, Teff, calcula tudo ate a distancia */

int metododireto;
float use_teff, use_dnu, use_numax, use_lg, use_pspacing,
  ...
  */
  while (fscanf (fp_inp,  
	...
	fscanf(fp_inp, "%f %f", &magobs.fil[ic], &smagobs.fil[ic]) ;     
      ...
      printf ("\n Using luminosity: L/Ls=%.3f+-%.3f, log(L/Ls)=%.3f+-%.3f\n", lum, elum, log_lum, slog_lum);
#endif
      ...
      if (metododireto==0 && print_input_pdfs==1) 
	{
	  ...
	  if (use_dnu>0.0) set_pdf(&pdf_deltanu, 0, (dnu-10.0 <0) ? 0.0 : (int) (dnu-10.0),
				   (int) (dnu+10.0), 0.005);
	  if (kind_tracks==3 && use_pspacing>0.0)
	    set_pdf(&pdf_pspacing, 0, (pspac-200.0 <0) ? 0.0 : (int) (pspac-200.0),
				   (int) (pspac+200.0), 0.1); 
	  if (use_numax>0.0) set_pdf(&pdf_numax0, 0, (numax-50.0 <0) ? 0.0 : (int) (numax-50.0),
				     (int) (numax+50.0), 0.05);
	  // printf("\nDnu min %f, max %f \n", pdf_deltanu.xmin, pdf_deltanu.xmax);
	  //printf("\nnumax min %f, max %f \n", pdf_numax0.xmin, pdf_numax0.xmax);
	}
      //JPDFS
      if (print_jpdfs==1)
	{
      ...
      if (ext_coef_table==1)
	{
      ...
      PDF pdf_loglum;
      float use_lum = 1.0;
      set_pdf(&pdf_loglum, 1, -1.0, 4.0, 0.002);
#endif
	      ...
	      for (lage=pdf_age.xmin, j=0; j<pdf_age.n; lage+=pdf_age.dx, j++) 
		{
		  ...
		  if (kind_tracks==3) // only for seismic tracks: MESA
		    {
		      ...
		      deltanu = isoc->star[0].ddeltanu * fdeltanu(mass_act, pow(10.0, isoc->star[0].logr)); // interpolating Dnu/DnuSR
		  ...
		  for (i=0; i<isoc->npun-1; i++)
		      ...
		      if (kind_tracks==3) // only for seismic tracks: MESA
			{
			  ...
			  // (float)n_int; 
			  delta_dnu = (isoc->star[i+1].ddeltanu *
				       fdeltanu(isoc->star[i+1].mass, pow(10.0, isoc->star[i+1].logr)) -
				       isoc->star[i].ddeltanu *
				       fdeltanu(isoc->star[i].mass, pow(10.0, isoc->star[i].logr)) ) / (float)n_int; 
...
#endif
#ifdef USING_LUM
				add2pdf (&pdf_loglum, logl, nstelle*weight);
#endif
				if (print_jpdfs==1)
				  {
  ...
  if (stat ("param.conf", &stt) == 0)
    {
      ...
      fscanf (fp_conf, "%f %f %f %f %f", &use_teff, &use_lg, &use_dnu, &use_numax, &use_pspacing);
      fgets(lixo, 80, fp_conf);
  ...
  else
    {
      printf ("\nDefault param.conf:");
      metododireto = 0;
      use_teff = 1.0; use_lg = 0.0; use_dnu = 1.0; use_numax = 1.0; use_pspacing = 0.0;
