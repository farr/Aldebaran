...
\startdata
\cutinhead{Per-Site RV Offset and Uncertainty Scaling}
$\mu_j$ & $N\left[ \left\langle y_{j} \right\rangle, 10 \sigma_{y_{j}} \right] \left( \mu _j \right)\tablenotemark{a}$ & The per-site RV offset. \\
$\nu_j$ & $1/\nu_j, \quad \nu_j \in [1/2, 2]$ & The per-site uncertainty scale factor. \\
...
$e$ & $2e$\tablenotemark{b} & Keplerian eccentricity \\
$\omega$ & $\mathrm{const}$ & Longitude of pericentre \\
$\chi$ & $\mathrm{const}$ & Pericentre passage is at $t = \chi P$\\
\enddata
\tablenotetext{a}{$\left\langle y_j \right\rangle$ and $\sigma_{y_j}$ are the mean and standard deviation of the RV timeseries from site $j$.}
